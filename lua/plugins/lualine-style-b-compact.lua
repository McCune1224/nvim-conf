-- ============================================================================
-- Lualine Style B: "Compact" ═══
-- Efficient layout: winbar for file context, statusline for project info
-- Compact components, thin separators, no visual clutter
-- ============================================================================
...
-- Compact diagnostics: just counts without icons
local function compact_diagnostics()
  ...
  for i = 1, 4 do
    ...
    if count > 0 then table.insert(parts, labels[i] .. count) end
  end
...
-- Compact position: just line number
local function compact_position()
...
-- ── Per-buffer components ──

lualine.setup({
  options = {
    theme = 'auto',
    -- Thin clear separators — minimal but defined section boundaries
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- WINBAR: File path + git info (always visible, like VS Code title)
  -- ═══════════════════════════════════════════════════════════════
  winbar = {
    lualine_a = {
  ...
  -- ═══════════════════════════════════════════════════════════════
  -- BOTTOM BAR: Compact — mode, LSP, diagnostics, position
  --   a: mode    b: aerial    c: filename    x: diag    y: position    z: progress
  -- ═══════════════════════════════════════════════════════════════
  sections = {
    lualine_a = {
    ...
    lualine_c = {
      { filename_only, padding = { left = 1, right = 1 } },
    },
    lualine_x = {
      { compact_diagnostics, padding = { left = 1, right = 1 } },
    },
    lualine_y = {
      { compact_position, padding = { left = 1, right = 1 } },
    },
    lualine_z = {
